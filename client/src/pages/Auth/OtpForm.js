...
const OtpForm = ({ userDetails, updateParentState }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post(`/api/v1/auth/verifyforgototp/${value.code}`, otpAuth.user);
            if (res.data.success) {
    ...
    const resendOtp = async () => {
        try {
            const res = await axios.post(`/api/v1/auth/forgot-password/${otpAuth.user.email}`, otpAuth.user);
            if (res.data.success) {
